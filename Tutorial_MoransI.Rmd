...
<summary>If you're doing lab 5 you DO NOT NEED TO DO THIS AGAIN</summary>
...
Lots of examples here: https://sfdep.josiahparry.com/

As described in lectures, there are many ways to calculate neighbourhood. Here are a few of them. They all rely on getting the geometry of each polygon and our choice of centroid

```{r}
# First get the geometry/coords and the central point in each polygon
...
To calculate and plot the three nearest neighbours (nearest centroids)
...
plot(neighbor.nearest3, st_coordinates(ACS_county.centroid), 
...
To calculate and plot adjacency neighbors:
...
To calculate and plot second order weights


```{r}
st_nb_lag_cumul(neighbor.rooks, 2)
...
We can take a quick look using this plot command

```{r}

plot(st_geometry(ACS_county.sf), border='blue')

plot(neighbor.queens, st_coordinates(st_centroid(ACS_county.sf)), 
