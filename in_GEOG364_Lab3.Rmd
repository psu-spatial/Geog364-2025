...
For more choice and built-in colourblind checking, we can use the new cols4all package.
 ...
 - Go to the Packages tab, click install and install `cols4all`
 - Go to the Packages tab, click install and install `shinyjs`
 - Now go to your library code chunk at the top, add `library(cols4all)` and run
...
**`tm_shape(house_sf)`** – tells R which spatial dataset you want to plot. It sets up the base layer (in this case, your points). You must always use `tm_shape()` before adding any drawing layer.
...
**`tm_dots(...)`** – adds point symbols to the map for each feature in `house_sf`. You can customise how these look:

- `fill = "Distance.Station"` – the variable used to colour the points. This must be numeric or categorical.
- `size = 6` – sets the size of the point symbols. Try values between 2 and 8 depending on how crowded the map looks.
- `fill.scale = distance_color_scale` – tells R which palette and classification system to use. This object should be created with `tm_scale_intervals()`.
- `fill.legend = tm_legend("Distance to station (km)")` – adds a clear legend title. Use something meaningful with units.

**`tm_basemap(server = "CartoDB.VoyagerNoLabels")`** – adds a clean background map for geographic context. This version has no labels, which is useful if your points are dense or you plan to add your own labels. Basemaps come from online tile providers and help you see where your data is located in the world.
